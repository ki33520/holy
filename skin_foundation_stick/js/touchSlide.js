...
var TouchSlide = function(a){
	...
	a = a||{};
	var opts = {
		slideW:a.slideW || '1000',
		slideCell:a.slideCell || "#touchSlide", //运行效果主对象，必须用id！，例如 slideCell:"#touchSlide"
		titCell:a.titCell || ".hd li", // 导航对象，当自动分页设为true时为“导航对象包裹层”
		mainCell:a.mainCell || ".bd", // 切换对象包裹层
		effect:a.effect || "left", // 效果，支持 left
	...
	var slideH=0;
	var slideW=0;// mainCell滑动距离，后面会覆盖
	var selfW=0;
	var scrollY ;
	//取最大值
	for ( var i =0; i<conBoxSize; i++ ){  
			...
			var chiW = opts.slideW - pa;
			var chiSW = opts.slideW + ma;
			if( chiW>selfW ){ selfW=chiW; slideW=chiSW;  }
			//if( chiW>selfW ){ selfW=chiW;slideW = opts.slideW;}
		}
	...
	var doPlay=function(isTouch){
		...
		{
			removeClass(conBox.children[i],opts.titOnClassName)
			removeClass(navObj[i],opts.titOnClassName);
			if( i == index ){ addClass(navObj[i],opts.titOnClassName);addClass(conBox.children[i],opts.titOnClassName)}
		}
