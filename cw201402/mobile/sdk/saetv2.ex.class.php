<?php
if ($_SERVER['SERVER_ADDR'] == '112.124.19.200') {//测试
	define('BASEPATH', 'http://ellady.iprotime.com/');
	define("WB_AKEY", '2193710618');
	define("WB_SKEY", '3a0bc08595dbe44813271fc3492bbaa1');
	define("WB_CALLBACK_URL", BASEPATH . 'cw201402/mobile/callback.php?t=s');
}
else {//正式
	define('BASEPATH', 'http://www.el-lady.com.cn/');
	define("WB_AKEY", '825602565');
	define("WB_SKEY", 'c8e94f33d285b9260c9d057cefdeb04e');
	define("WB_CALLBACK_URL", BASEPATH . 'cw201402/mobile/callback.php?t=s');
}
...
class elAnr2SupportClient {
    ...
    public function __construct($kx_ak=FALSE, $kx_as=FALSE, $rr_ak=FALSE, $rr_as=FALSE) {
        if ($kx_ak && $kx_as) {
            $this->kx_ak = $kx_ak;
            $this->kx_as = $kx_as;
        } elseif ($rr_ak && $rr_as) {
            ...
            $this->kx_ak = '523143224758996311b260f6478d32b3';
            $this->kx_as = '05f83e17812576c3e17f6022a8e44724';
            $this->rr_ak = '9c282d58319a451eaf7cf884a880f18c';
            $this->rr_as = '2493e1c668254e7d84a16460b748840f';
        }
    }

    public function getOauthPath($type, $redirect_uri, $scope=array('basic')) {
        switch ($type) {
            case 'kx':
                $query = array(
                    'response_type' => 'code',
                    'client_id' => $this->kx_ak,
                    'redirect_uri' => $redirect_uri, 
                    'scope' => implode(' ', $scope),
                );
                return 'http://api.kaixin001.com/oauth2/authorize?' . http_build_query($query);
                break;
            case 'rr':
                $query = array(
                    'client_id' => '9c282d58319a451eaf7cf884a880f18c',
                    'response_type' => 'code',
                    'redirect_uri' => $redirect_uri,
                    'scope' => implode(' ', $scope),
                    'x_renew' => TRUE
                );
                return 'https://graph.renren.com/oauth/authorize?' . http_build_query($query);
                break;
            default:
                break;
        }
    ...
    public function getAccessTokenPath($type, $redirect_uri, $code) {
        switch ($type) {
            case 'kx':
                $query = array(
                    ...
                    'client_id' => $this->kx_ak,
                    'redirect_uri' => $redirect_uri,
                    'client_secret' => $this->kx_as,
                    'code' => $code
                );
                return 'https://api.kaixin001.com/oauth2/access_token?' . http_build_query($query);
                break;
            case 'rr':
                $query = array(
                    'grant_type' => 'authorization_code',
                    'client_id' => $this->rr_ak,
                    'redirect_uri' => $redirect_uri,
                    'client_secret' => $this->rr_as,
                    'code' => $code
                );
                return 'https://graph.renren.com/oauth/token?' . http_build_query($query);
                break;
            default:
                break;
        }
    ...
    public function getMe($type, $access_token) {
        switch ($type) {
            case 'kx':
                $query = array(
                    'access_token' => $access_token,
                );
                $file = file_get_contents('https://api.kaixin001.com/users/me.json?' . http_build_query($query));
            ...
            case 'rr':
                ...
                $args['sig'] = $this->generateRrSignature($args);
                $options[CURLOPT_POSTFIELDS] = http_build_query($args);
                $options[CURLOPT_URL] = 'http://api.renren.com/restserver.do';
                $ch = curl_init();
                curl_setopt_array($ch, $options);
                $result = curl_exec($ch);
                curl_close($ch);
                if ($result) {
                    $result = array_shift(json_decode($result));
                }
                return $result;
                break;
            default:
    ...
    public function getFriends($type, $access_token) {
        $options = array(
            CURLOPT_POST => 1,
            CURLOPT_SSL_VERIFYPEER => FALSE,
            CURLOPT_RETURNTRANSFER => TRUE
        ...
        switch ($type) {
            case 'kx':
                ...
                $result = file_get_contents('https://api.kaixin001.com/friends/me.json?' . http_build_query($args));
                $result = json_decode($result);
                $next = $result->next;
                $friends = $result->users;
                for ($index = 1; $index < ceil($result->total / 50); $index++) {
                    $args = array(
                        'access_token' => $access_token,
                        'num' => 50,
                        'start' => $next
                    ...
                    $result = json_decode($result);
                    $next = $result->next;
                    $friends = array_merge($result->users, $friends);
                }
...
//                } while ($next < $result->total);
    ...
    public function generateRrSignature($params) {
        ksort($params);
        reset($params);
        $str = '';
